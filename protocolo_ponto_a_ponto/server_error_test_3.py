...
# ERRO: Transmissão com ausência de resposta de handshake, por mais de 20 segundos
# linha 69
...
def main():
    try:
        ...
        # esperando mensagem
            ocioso = True
            # (ERRO) forcei o tipo da mensagem pra ser errado pra não haver checagem certa
            tipo = 2
            while ocioso:
                timer_erro = time.time()
                print('estou ocioso')
                ...
                # (ERRO) Após 20 segundos, o handshake volta ao normal
                if time.time() - timer_erro > 20:
                    tipo = 1
                if tipo_msg == tipo: # (ERRO) De 1 colequei 2 para forçar o erro do handshake, assim o cliente nunca vai receber a confirmação
                    id_servidor = head[2]
